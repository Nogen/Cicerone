...
public class testRicercaByTag {
	...
	public void setUp() {
		...
		luogo = "Bitonto";
		when(att1.toString()).thenReturn("Bari, Bari, Italia");
		when(att2.toString()).thenReturn("Bitonto, Bari, Italia");
		when(att3.toString()).thenReturn("bitonto, bari, italia");
		lista.add(att1);
		lista.add(att2);
		lista.add(att3);
		ricercaPadre = mock(IGestoreRicerca.class);
		when(ricercaPadre.ricerca()).thenReturn(lista);
		ricerca1 = new RicercaByTag(ricercaPadre, luogo);
	}
