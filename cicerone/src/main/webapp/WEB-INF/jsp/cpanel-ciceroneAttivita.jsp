...
<body>
	...
	<div id="mySidenav" class="sidenav">
		...
		<div class="container">
			...
			<div class="row filter active">
				<div class="col mt-2 mb-2">
					<a href="attivitaCreate"><img alt="Image here"
						src="icons/operations.png" class="dimIcons mr-3">Attivit&#224;
						create</a>
				</div>
			</div>
			<div class="row filter">
				<div class="col mt-2 mb-2">
					<a href="creaAttivita"><img alt="Image here"
						src="icons/notification.png" class="dimIcons mr-3">Nuova
						Attivit&#224;</a>
				</div>
			</div>
			<div class="row filter">
			...
			<div class="row filter">
				<div class="col mt-2 mb-2">
					<a href="chooseGlobetrotter"><img alt="Image here"
	...
	<div class="fuckmargin" id="main">
		<div class="container-fluid">
			...
			<div class="row mt-5 no-gutters justify-content-md-center">
				<c:forEach items="${listaAttivita}" var="attivita">
					<div class="col-12 col-lg-9 viewclass mb-5 mr-lg-0 ">
						<div class="card text-left shadow">
							...
							<div class="row mt-3">
								...
								<div class="col-12 col-lg-6 mt-3">
									<input readonly class="form-control-plaintext">${e:forHtml(attivita.luogo)}</label>
								</div>
							</div>
							<div class="row mt-3">
								<div class="col-12 col-lg-6 mt-3">
								...
								<div class="col-12 col-lg-6 mt-3">
									<input readonly class="form-control-plaintext">${e:forHtml(attivita.dataIncontro)}</label>
								</div>
							</div>
							<div class="row mt-3">
								<div class="col-12 col-lg-6 mt-3">
								...
								<div class="col-12 col-lg-6 mt-3">
									<input readonly class="form-control-plaintext">${e:forHtml(attivita.oraIncontro)}</label>
								</div>
							</div>
							<div class="row mt-3">
								<div class="col-12 col-lg-6">
							...
							<div class="card-footer">
								<div class="row no-gutters">
									<c:if test="${attivita.statoAttivita != 'chiuso'}">
												...
												<div class="modal-content">
													<div class="modal-header">
														<h5 class="modal-title" id="exampleModalLabel">Conferma
															cancellazione attivit&agrave;</h5>
														<button type="button" class="close" data-dismiss="modal"
															aria-label="Close">
															<span aria-hidden="true">&times;</span>
														</button>
													</div>
													<div class="modal-body">Sei sicuro di volere
														cancellare questa attivit&agrave;?</div>
													<div class="modal-footer">
														<button type="button" class="btn btn-secondary"
															data-dismiss="modal">Annulla</button>
														<a
															href="cancellaAttivita?id=${e:forUriComponent(attivita.idAttivita)}"
															class="btn btn-primary text-light">Conferma</a>
													</div>
												</div>
											</div>
										</div>
										...
										<div class="col-12 col-lg-6 text-center mt-1">
											<a
												href="modificaAttivita?id=${e:forUriComponent(attivita.idAttivita)}"
												class="btn btn-primary text-light">Modifica</a>
										</div>

										<div class="col-12 col-lg-6 text-center mt-1">
											<a
												href="partecipanti?id=${e:forUriComponent(attivita.idAttivita)}"
												class="btn btn-primary text-light">Partecipanti</a>
										</div>
									</c:if>
									<div class="col-12 col-lg-6 text-center mt-1">
										<a
											href="visualizzaAttivita?id=${e:forUriComponent(attivita.idAttivita)}"
											class="btn btn-primary text-light" role="button">Dettagli</a>
									</div>
								</div>
							</div>
						</div>
			...
			<nav aria-label="Page navigation">
				<ul class="pagination justify-content-center mt-3">
					<li
						...
						class="page-link"
						href="attivitaCreate?p=${pageIndex-1}&orderBy=${orderBy}&byLingua=${e:forUriComponent(byLingua)}&byLuogo=${e:forUriComponent(byLuogo)}&byDate=${e:forUriComponent(byDate)}&byKey=${e:forUriComponent(byKey)}">Previous</a></li>
					<c:forEach begin="1" end="${pageSize}" varStatus="loop">
						<li
							class="page-item  <c:if test="${pageIndex == loop.index - 1}"> active</c:if>"><a
							class="page-link"
							href="attivitaCreate?p=${loop.index-1}&orderBy=${orderBy}&byLingua=${e:forUriComponent(byLingua)}&byLuogo=${e:forUriComponent(byLuogo)}&byDate=${e:forUriComponent(byDate)}&byKey=${e:forUriComponent(byKey)}">${loop.index}</a></li>
					</c:forEach>
					<li
						class="page-item <c:if test="${pageIndex == (pageSize-1)}"> disabled </c:if>"><a
						class="page-link"
						href="attivitaCreate?p=${pageIndex+1}&orderBy=${orderBy}&byLingua=${e:forUriComponent(byLingua)}&byLuogo=${e:forUriComponent(byLuogo)}&byDate=${e:forUriComponent(byDate)}&byKey=${e:forUriComponent(byKey)}">Next</a></li>
				</ul>
			</nav>
			</c:if>
		</div>
...
<footer>
	<form action="creaAttivita">
		<button type="submit" class="fab">
			<img alt="Image here" src="icons/fab.png">
		</button>
	</form>
</footer>
